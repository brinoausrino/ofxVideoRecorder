...
#endif
#ifdef TARGET_WIN32
	void setup(HANDLE pipeHandle, lockFreeQueue<shared_ptr< ofPixels>> * q);
...
private:
	...
	string filePath;
	int fd;
	lockFreeQueue<shared_ptr<ofPixels>> * queue;
...
public:
	...
	void setFfmpegLocation(string loc) {
		ofDirectory dir(loc);
		ffmpegLocation = dir.getAbsolutePath() + "\\ffmpeg.exe"; 
	}
...
private:
	...
	lockFreeQueue<shared_ptr<ofPixels>> frames;
