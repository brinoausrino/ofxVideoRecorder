...
void ofxVideoDataWriterThread::setup(HANDLE videoHandle_, lockFreeQueue<shared_ptr<ofPixels>> * q) {
	queue = q;
	bIsWriting = false;
	bClose = false;
...
void ofxVideoDataWriterThread::threadedFunction() {
	...
	{
		//ofPixels * frame = NULL;
		shared_ptr<ofPixels>  frame = nullptr;
...
bool ofxVideoRecorder::setupCustomOutput(int w, int h, float fps, int sampleRate, int channels, string outputString, bool sysClockSync, bool silent)
{
	...
	bRecordAudio = false; // (sampleRate > 0 && channels > 0);
	...
	if (bRecordAudio && bRecordVideo) {
		...
		vCmd << " -i " << convertWideToNarrow(vPipename) << " -vcodec " << videoCodec;
		if (videoBitrate != "") {
			vCmd << " -b:v " << videoBitrate;
		} 
...
bool ofxVideoRecorder::addFrame(const ofPixels &pixels)
{
	...
	if (bIsInitialized && bRecordVideo)
	{
		...
		for (int i = 0; i < framesToAdd; i++) {
			...
			frames.Produce(shared_ptr<ofPixels>(new ofPixels(pixels)));
			videoFramesRecorded++;
		}
		videoThread.signal();
		return true;
...
void ofxVideoRecorder::clearFrames() {
	frames.clear();
}
